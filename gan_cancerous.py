...
# Paths to your PCam H5 files
x_path = '/content/camelyonpatch_level_2_split_train_x.h5'
y_path = '/content/camelyonpatch_level_2_split_train_y.h5'

# Output folder
output_dir = '/content/cancerous_images'
os.makedirs(output_dir, exist_ok=True)

# Open the HDF5 files
with h5py.File(x_path, 'r') as fx, h5py.File(y_path, 'r') as fy:
    images = fx['x']      # shape: (N, 96, 96, 3), dtype=uint8
    labels = fy['y'][:]   # shape: (N,), values ∈ {0,1}

    # Iterate and save only healthy ones
    for idx in range(len(labels)):
        if labels[idx] == 1:
            img_array = images[idx]            # H×W×C numpy array
            img = Image.fromarray(img_array)   # convert to PIL Image
            # save with zero-padded index
            img.save(f'{output_dir}/healthy_{idx:06d}.png')

print(f"Saved {len(os.listdir(output_dir))} healthy patches to {output_dir}")
device = 'cuda'
z_dim = 128
batch_size = 512
import glob
...
dataset = PNGFolderDataset("/content/cancerous_images", transform=transform)
...
for epoch in range(epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        torch.save(generator.state_dict(), 'best_generator_cancerous.pth')
        torch.save(discriminator.state_dict(), 'best_discriminator_cancerous.pth')
...
gen.load_state_dict(torch.load('/content/best_generator_cancerous.pth', map_location=device))
disc.load_state_dict(torch.load('/content/best_discriminator_cancerous.pth', map_location=device))
